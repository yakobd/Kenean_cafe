...
export default function WaiterFeedbackPage() {
  ...
  const loadFeedback = () => {
    ...
    // Filter feedback for orders this waiter was involved in
    const waiterFeedback = allFeedback.filter(
      (f) => f.waiterName === waiterName && !f.archived
    );

    // Sort: pinned first, then by timestamp (newest first)
    const sorted = waiterFeedback.sort((a, b) => {
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-5 mb-6">
          ...
          <p className="text-gray-300 text-sm">
            View feedback from customers you've served. This is read-only - only admins can manage feedback.
          </p>
        </div>
