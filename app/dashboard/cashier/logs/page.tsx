...
export default function CashierLogsPage() {
  ...
  const loadOrders = () => {
    ...
    // Filter orders handled by this cashier
    const cashierOrders = history.filter((order: Order) =>
      order.billApprovedByCashier?.cashierName === cashierName ||
      order.paymentConfirmedByCashier?.cashierName === cashierName
    );

    setAllOrders(cashierOrders.reverse());
    filterByDateRange(cashierOrders);
                ...
                >
                  ...
                  {expandedOrderId === order.id && (
                    <div className="border-t border-gray-700 bg-gray-900 p-5">
                      ...
                      {/* Audit Trail */}
                      {(order.billApprovedByCashier || order.paymentConfirmedByCashier) && (
                        <div className="bg-gray-800 rounded-lg p-4 mb-4">
                          <h4 className="text-sm font-bold text-white mb-3">Your Actions</h4>
                          <div className="space-y-2 text-sm">
                            {order.billApprovedByCashier && (
                              <div className="text-gray-300">
                                ✓ Bill approved at {new Date(order.billApprovedByCashier.timestamp).toLocaleTimeString()}
                              </div>
                            )}
                            {order.paymentConfirmedByCashier && (
                              <div className="text-gray-300">
                                ✓ Payment confirmed at {new Date(order.paymentConfirmedByCashier.timestamp).toLocaleTimeString()}
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {feedbackMap[order.id] && (
                        <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
                          <div className="flex items-center justify-between">
                            ...
                            <Link
                              href="/dashboard/cashier/feedback"
